...
class King extends Piece {
  ...
  for(let i = 0; i< validMoves.length; i++){
    ...
    if(dummyPiece == null)
    continue;
    if((dummyPiece.type === "bishop" || dummyPiece.type === "queen" ) && dummyPiece.color != this.color){
      return true;
    }
  }
  ...
  for(let i = 0; i< validMoves.length; i++){
    ...
    if(dummyPiece == null)
    continue;
    if( (dummyPiece.type === "rook" || dummyPiece.type === "queen" ) && dummyPiece.color != this.color){
      return true;
    }
  }
  ...
  let pawnSquares = [];
  if(this.number+ direction >=0 && this.number + direction <=7 && this.convertLetterToNumber(this.letter) -1 >=0 ){
    pawnSquares.push([this.number + direction, this.convertLetterToNumber(this.letter) -1]);
  }
  if(this.number+ direction >=0 && this.number + direction <=7 && this.convertLetterToNumber(this.letter) +1 <=7 ){
    pawnSquares.push([this.number+ direction, this.convertLetterToNumber(this.letter) +1]);
  }
  for(let i =0; i< pawnSquares.length; i++){
    if(board[pawnSquares[i][0], pawnSquares[i][1]].piece != null && board[pawnSquares[i][0], pawnSquares[i][1]].piece.type === "pawn" && board[pawnSquares[i][0], pawnSquares[i][1]].piece.color != this.color){
      return true;
    }
  }
}
...
isCheckMate(board){
  ...
  for(let i = 0; i< validMoves.length; i++){
    let dummyKing = new King(this.color, validMoves[i][0], validMoves[i][1]);
